...
public class HoneyPotOpgD extends IRElementVisitor<Integer> {

	private ArrayList<MacIp> ips = new ArrayList<MacIp>();
	private ArrayList<MacIp> intrusions = new ArrayList<MacIp>();
	...
	public Integer visitEntries(compiler.IR.NWEntries e) throws VisitorException {
		...
		for (int i = 0; i < intrusions.size(); i++) {
			
//			System.out.println(intrusions.get(i).ipv4adr + "changed from " + ips.);	
		}
	...
	public static void print(IR ir) {
		try {
			new HoneyPotOpgD().visitEntries(ir.e); 
